...
class AdmitadTracker private constructor(
    ...
    @Nullable callback: web.browser.dragon.huawei.utils.appsflyer.TrackerInitializationCallback
) {
    // tracking controller
    ...
    fun logPurchase(order: web.browser.dragon.huawei.utils.appsflyer.AdmitadOrder, @Nullable trackerListener: ru.tachos.admitadstatisticsdk.TrackerListener?) {
    ...
    @JvmOverloads
    fun logPurchase(
        order: web.browser.dragon.huawei.utils.appsflyer.AdmitadOrder,
        channel: String? = ADMITAD_MOBILE_CHANNEL,
        @Nullable trackerListener: ru.tachos.admitadstatisticsdk.TrackerListener? = null
    ) {
        controller!!.track(
